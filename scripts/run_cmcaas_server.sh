...
if [ -e /dev/sgx_enclave ] && [ -e /dev/sgx_provision ]; then
    ...
    docker run -it --device /dev/sgx_enclave --device /dev/sgx_provision -p 8080:8080 --name "$IMAGE_NAME" "$IMAGE_NAME:latest"
else
    echo "[!] SGX device files not found. Running in simulation mode..."
    docker run -it -p 8080:8080 --name "$IMAGE_NAME" "$IMAGE_NAME:sim"
fi
