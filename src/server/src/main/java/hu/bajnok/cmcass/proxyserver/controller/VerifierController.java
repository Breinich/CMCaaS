...
public class VerifierController {
  ...
  /**
   * Initiate enclave process This request starts an enclave process attached to the authenticated
   * user's ID
   *
   ...
   */
  @GetMapping("/init")
  public ResponseEntity<String> initEnclave(@AuthenticationPrincipal UserDetails user)
      throws Exception {
  ...
  public ResponseEntity<String> agreeWithEnclave(
  ...
  public ResponseEntity<String> getEnclaveQuote(
      @AuthenticationPrincipal UserDetails user,
      @RequestParam("publicKey") String processKey,
  ...
  public ResponseEntity<byte[]> getEncryptedResults(
      @RequestParam("publicKey") String processKey, @AuthenticationPrincipal UserDetails user) {
    String username = (user != null) ? user.getUsername() : null;
    if (username == null) {
    ...
    logger.info(
        "User [{}] is requesting the encrypted results for enclave with public key {}.",
        username,
        processKey);
    ...
    try {
      ...
      return ResponseEntity.badRequest()
          .body(("Error retrieving the results: " + e.getMessage()).getBytes());
    }
  }
