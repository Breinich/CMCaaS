public class Test {
  ...
  public static long create_a_formula(long env) {
    ...
    for (int i = 0; i < vn.length; ++i) {
      vars[i] =
          mathsat.api.msat_declare_function(env, vn[i], mathsat.api.msat_get_rational_type(env));
      assert (!mathsat.api.MSAT_ERROR_DECL(vars[i]));
    }
    for (int i = 0; i < fn.length; ++i) {
      long r = mathsat.api.msat_get_rational_type(env);
      long[] at = {r};
      long tp = mathsat.api.msat_get_function_type(env, at, at.length, r);
    ...
    tmp =
        mathsat.api.msat_make_leq(
            env,
            mathsat.api.msat_make_constant(env, vars[1]),
            mathsat.api.msat_make_constant(env, vars[0]));
    res = mathsat.api.msat_make_and(env, res, tmp);
    tmp =
        mathsat.api.msat_make_not(
            env,
            mathsat.api.msat_make_equal(
                env,
                mathsat.api.msat_make_constant(env, vars[6]),
                mathsat.api.msat_make_constant(env, vars[7])));
    res = mathsat.api.msat_make_and(env, res, tmp);
    tmp = mathsat.api.msat_from_string(env, "(and (<= v0 v1) (= v2 (- v3 v4)))");
    res = mathsat.api.msat_make_and(env, res, tmp);
  ...
  public static void example1() {
    ...
    System.out.println("Environment is inconsistent, " + "retracting constraint " + s);
    ...
    System.out.print("Environment is now consistent, " + "getting the model...\n");
  ...
  static void example_interpolation() {
    ...
    for (int i = 0; i < vars.length; ++i) {
      long d =
          mathsat.api.msat_declare_function(env, vars[i], mathsat.api.msat_get_rational_type(env));
      assert (!mathsat.api.MSAT_ERROR_DECL(d));
    }
    for (int i = 0; i < ufs.length; ++i) {
      long r = mathsat.api.msat_get_rational_type(env);
      long[] tpa = {r};
      long tp = mathsat.api.msat_get_function_type(env, tpa, tpa.length, r);
    ...
    System.out.println("Asserted formula A (in group " + Integer.toString(group_a) + "): " + s);

    /* create and assert formula B */
    formula =
        mathsat.api.msat_from_string(env, "(and (= x2 (g b)) (= y2 (g b)) (<= x1 y1) (< x3 y3))");
    assert (!mathsat.api.MSAT_ERROR_TERM(formula));
    ...
    assert (res == 0);
    s = mathsat.api.msat_term_repr(formula);
    System.out.println("Asserted formula B (in group " + Integer.toString(group_b) + "): " + s);

    if (mathsat.api.msat_solve(env) == mathsat.api.MSAT_UNSAT) {
      int[] groups_of_a = {group_a};
      long interpolant = mathsat.api.msat_get_interpolant(env, groups_of_a, 1);
